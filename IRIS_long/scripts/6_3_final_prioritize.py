...
tissue_percentage_cutoff = float(sys.argv[4])
...
with open("%s/6_2_peptide_matrix_WindowSize_%s_TCR.txt" % (outf_dir, window_size), "r") as inf_s1:
	for index,line in enumerate(inf_s1):
		...
		### Hydrophobicity and Isoelectric point filtering
		X = ProteinAnalysis(arr[0])
		peptide2iso_elec_dict[arr[0]] = float(X.isoelectric_point())
		peptide2hydro_dict[arr[0]] = float(X.gravy())
...
with open("%s/6_1_Summarized_TCR_netMHCpan_reshaped_out.txt" % outf_dir, "r") as inf:
	for index,line in enumerate(inf):
		...
		outf.write('\t'.join(arr)+'\t'+str(peptide2hydro_dict[peptide])+'\t'+str(peptide2iso_elec_dict[peptide])+'\t'+str(peptide2score_mean_dict[peptide])+'\t'+str(peptide2score_median_dict[peptide])+'\t'+';'.join(this_pep_match_trans_list)+'\t'+';'.join(this_pep_match_gene_list)+'\t'+';'.join(this_pep_match_gene_name_list)+'\t'+';'.join(this_pep_pos)+'\t'+';'.join(this_genome_pos)+'\t'+log2_CPM_dict[peptide]+'\n')
outf.close()
