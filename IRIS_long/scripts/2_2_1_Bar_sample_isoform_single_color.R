...
uniprot_inf_name = args[9] 
basic_inf_name = args[10] 
...
reference_list <- c("Melanocytic","Transitory","Neural_crest_like","Undifferentiated","Tissue")
...
if (identical(as.character(unique(gene_data$Subtype)), reference_list)) {
	f_fig <- f_fig + scale_colour_manual(name="Group",values = group_color_list, breaks=c("Melanocytic","Transitory","Neural_crest_like","Undifferentiated","Tissue"), labels=c("Melanocytic","Transitory","Neural crest-like","Undifferentiated","Tissue"))
} else {
...
if (identical(as.character(unique(gene_data$Subtype)), reference_list)) {
	f_fig_exp <- f_fig_exp + scale_colour_manual(name="Group",values = group_color_list, breaks=c("Melanocytic","Transitory","Neural_crest_like","Undifferentiated","Tissue"), labels=c("Melanocytic","Transitory","Neural crest-like","Undifferentiated","Tissue"))
} else {
	f_fig_exp <- f_fig_exp + scale_colour_manual(name="Group",values = group_color_list)
}
f_fig_exp <- f_fig_exp + geom_bar(stat="identity", position = position_stack(vjust = 1, reverse = TRUE), width=0.9, color="black", size=0.4)
...
def_width = max(0.16*length(unique(gene_data$Sample)) + 1, 8)
