...
def fetch_seq(genome, chr, start, end, strand):
	...
	try:
		if strand == '-':
			seq = str(genome[chr].seq[start:end].reverse_complement())
		else:
			seq = str(genome[chr].seq[start:end])
	except:
		raise Exception('BioPython cannot fetch sequences')
...
def check_NMD(this_trans,full_trans,strand):
	...
	if ORF_len <= 150: # too short transcript doesn't have NMD
		return False
	# if pass all 3 tests
	return True
...
#### short-read ######
	...
	for line in inf_gtf_data:
		...
		if re.findall('reference_id',arr[8]):
			raw_ENST_ID = re.findall(r'reference_id "(.+?)"',arr[8])[0]
			raw_ENSG_ID = re.findall(r'ref_gene_id "(.+?)"',arr[8])[0]
		else:
			raw_ENST_ID = re.findall(r'transcript_id "(.+?)"',arr[8])[0]
			raw_ENSG_ID = re.findall(r'gene_id "(.+?)"',arr[8])[0]
...
for line in inf:
	...
	chrom = re.findall(r'(chr\w+)',this_exon[1])[0]
	...
	if (use_annotation == 'yes') and (ENST_ID in CDS_dict):   #if this ENST is annotated, we use its annotated ORF
		...
		if len(final_seq) > 1000000: continue
