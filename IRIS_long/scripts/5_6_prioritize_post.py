	...
	for index,line in enumerate(cand_trans_inf_2):
		...
		if index == 0: 
			...
			for i in range(len(protein_seq)-window_size+1):
				...
				if pep2score_dict[this_peptide] >= cutoff_score:
			...
			if len(score_list) == 0:
				...
				## extracellular peptide length must more than 0 ##
				if len(overlap_pos) == 0:
					n_out_3 += 1
					continue
				elif len(overlap_pos) > 0:
					n_end += 1
...
print("Number of targets that have been filtered out in step2: Peptide specificity assessment\t"+str(n_out_2)+'\n')
print("Number of targets that have been filtered out in step3.5: Peptide surface presentation assessment\t"+str(n_out_3)+'\n')
print("Number of targets after filtering\t"+str(n_end)+'\n')
