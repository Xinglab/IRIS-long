...
######## get gene expression ######
gene_CPM_dict = defaultdict(lambda: defaultdict())
gene_sample_list = []
with open(gene_CPM_inf_name, 'r') as gene_CPM_inf:
	for index,line in enumerate(gene_CPM_inf):
		arr = line.strip().split('\t')
		if index == 0:
			gene_sample_list = arr[1:len(arr)]
		else:
			for i in range(1,len(arr)):
				gene_CPM_dict[arr[0]][gene_sample_list[i-1]] = float(arr[i])
...
for each_gene in Others_pro_dict:
	for each_sample in sample_list_defined:
		this_subtype = cell2subtype_dict[each_sample]
		if gene_CPM_dict[each_gene][each_sample] == 0:
			this_pro = 0.0
		else:
			this_pro = max(0.0, float(100-np.sum(list(map(float, Top_pro_dict[each_gene][each_sample].values())))))
...
print(len(Top_exp_dict),len(Others_exp_dict))
