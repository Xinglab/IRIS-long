...
def parse_args():
	...
	parser.add('-te', '--de_inf', dest='de_inf', type=str, help='tumor-enriched transcripts', required=True)
	parser.add('-ts', '--spe_inf', dest='spe_inf', type=str, help='tumor-specific transcripts', required=True)
...
with open(DE_inf, 'r') as inf_2:
	for index,line in enumerate(inf_2):
		...
		CPM_info_dict[arr[0]] = '\t'.join(arr)
		significant_dict[arr[0]] = 'Tumor-enriched'

with open(Spe_inf, 'r') as inf_1:
