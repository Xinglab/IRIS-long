...
with open(CDS_inf_name, 'r') as CDS_inf:
	for index,line in enumerate(CDS_inf):
		if line.startswith(">"):
			info_list = line.strip().split("_")
			if re.findall("PC|NMD", info_list[-1]):
				trans_ID = info_list[5]	
				if trans_ID in abundance_index_dict:
					CDS_dict[trans_ID] = info_list[7].split(";")
...
with open(bed_list_name,'r') as list_inf_2:
	for list_index,list_line in enumerate(list_inf_2):
		...
		if trans_ID in CDS_dict:
			initial_CDS_pos = (float(CDS_dict[trans_ID][0].split('#')[0])-1 - min_pos)
			...
			if len(CDS_dict[trans_ID]) >= 2:
				for i in range(len(CDS_dict[trans_ID])-1):
					...
					CDS_intron_len = (float(CDS_dict[trans_ID][i+1].split('#')[0])-1 - float(CDS_dict[trans_ID][i].split('#')[1]))
					current_CDS_exon_pos = str(current_CDS_pos)+':'+str(current_CDS_pos+CDS_exon_len)
					current_CDS_pos = current_CDS_pos + CDS_exon_len
					current_CDS_intron_pos = str(current_CDS_pos)+':'+str(current_CDS_pos+CDS_intron_len)
				...
				current_CDS_exon_pos = str(current_CDS_pos)+':'+str(current_CDS_pos+last_CDS_exon_len)
				corrected_CDS_exon_dict[trans_ID].append(current_CDS_exon_pos)
