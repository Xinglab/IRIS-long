...
outf_ID = open(outf_dir+'/4_4_'+key_word+'_detailed_match_ID.txt','w')
outf_fasta = open(outf_dir+'/4_4_'+key_word+'_PC.fasta','w')
outf_NMD_fasta = open(outf_dir+'/4_4_'+key_word+'_NMD.fasta','w')
outf_PC_NMD_fasta = open(outf_dir+'/4_4_'+key_word+'_PC_and_NMD.fasta','w')
outf_PTC_fasta = open(outf_dir+'/4_4_'+key_word+'_PC_PTC.fasta','w')
outf_NMD_PTC_fasta = open(outf_dir+'/4_4_'+key_word+'_NMD_PTC.fasta','w')
...
with open('%s/references/Processed_UniProt/UP000005640_9606.idmapping_coverted_2.txt' % dir_path,'r') as pro_inf:
	for line in pro_inf:
...
for line in inf:
	...
	if line.startswith('>'):
		...
		if flag == 'PC':
			if (re.findall(r'\*',line.upper())) or (re.findall('XXXXX',line.upper())):
				outf_PTC_fasta.write(rename_ID+'\n'+line.upper()+'\n')
			else:
				outf_fasta.write(rename_ID+'\n'+line.upper()+'\n')
				outf_PC_NMD_fasta.write(rename_ID+'\n'+line.upper()+'\n')
		elif flag == 'NMD':
			if (re.findall(r'\*',line.upper())) or (re.findall('XXXXX',line.upper())):
				outf_NMD_PTC_fasta.write(rename_ID+'\n'+line.upper()+'\n')
			else:
...
outf_NMD_PTC_fasta.close()
outf_PC_NMD_fasta.close()
