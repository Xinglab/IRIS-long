...
outf_dir = sys.argv[2]
outf = open(outf_dir+'/4_1_Gencode_converted_CDS_annotation.txt','w')
...
for line in gtf_inf:
	...
	if arr[2] == 'gene':
		...
		if re.findall('transcript_type',arr[8]):
			ENST_type = re.findall(r'transcript_type "(\w+)";',arr[8])[0]
		elif re.findall('transcript_biotype',arr[8]):
			ENST_type = re.findall(r'transcript_biotype "(\w+)";',arr[8])[0]
		raw_ENSG_ID = re.findall(r'gene_id "(.+?)";',arr[8])[0]
		if re.findall('_PAR_Y', raw_ENSG_ID):
			ENSG_ID = raw_ENSG_ID.split('.')[0]+'-PAR-Y'
		else:
		...
		#	tag = 'basic'
		tag = ';'.join(re.findall(r'tag "(.+?)";',arr[8]))
		strand = arr[6]
