...
def parse_args():
	...
	parser_figure.add_argument('-ip', '--isoform_proportion_inf', dest='isoform_proportion_inf', type=str, help='Isoform proportion infile', required=True)
	parser_figure.add_argument('-ic', '--isoform_cpm_inf', dest='isoform_cpm_inf', type=str, help='Isoform CPM infile', required=True)
	...
	parser_figure.add_argument('-be', '--bedgraph', dest='bedgraph', type=str, help='Generated bedgraph file for sample', required=True)
	...
	parser_difftest.add_argument('-ic', '--isoform_cpm_inf', dest='isoform_cpm_inf', type=str, help='Isoform CPM infile', required=True)
	parser_difftest.add_argument('-tn', '--tumor_num', dest='tumor_num', type=int, help='Number of tumor samples', required=True)
	parser_difftest.add_argument('-dep', '--detest_p', dest='detest_p', type=float, default=0.05, help='Cutoff of p-value in DE test (default = 0.05)')
	parser_difftest.add_argument('-det', '--detest_tumor_cpm', dest='detest_tumor_cpm', type=float, help='Cutoff of median CPM value in tumor samples, used to decide highly-expressed isoform (default = 3)', default = 3)
	parser_difftest.add_argument('-def', '--detest_fc', dest='detest_fc', type=float, help='Cutoff of fold change between tumor and normal, used to decide DE isoform (default = 2)', default = 2)
	parser_difftest.add_argument('-pp', '--pretest_p', dest='pretest_p', type=float, default=1e-6, help='Cutoff of p-value in prevalence test (default = 1e-6)')
	parser_difftest.add_argument('-ptc', '--pretest_tumor_cpm', dest='pretest_tumor_cpm', type=float, help='Cutoff of CPM value in tumor samples, used to decide whether an isoform is considered as expressed (default = 2)', default = 2)
	parser_difftest.add_argument('-pic', '--pretest_tissue_cpm', dest='pretest_tissue_cpm', type=float, help='Cutoff of CPM value in tissue samples, used to decide whether an isoform is considered as expressed (default = 1)', default = 1)
	...
	parser_translate.add_argument('-mo', '--mode', dest='mode', type=str, default = 'long-read', help='Long-read RNA-seq data mode', required=True)
	parser_translate.add_argument('-tg', '--trans_gtf', dest='trans_gtf', type=str, help='generated gtf file', required=True)
	parser_translate.add_argument('-ic', '--isoform_cpm_inf', dest='isoform_cpm_inf', type=str, help='Isoform CPM infile', required=True)
	...
	parser_translate.add_argument('-rg', '--ref_gtf', dest='ref_gtf', type=str, help='reference gencode annotation', required=True)
	...
	parser_car_t.add_argument('-ic', '--isoform_cpm_inf', dest='isoform_cpm_inf', type=str, help='Isoform CPM infile', required=True)
	parser_car_t.add_argument('-ip', '--isoform_proportion_inf', dest='isoform_proportion_inf', type=str, help='Isoform proportion infile', required=True)
	...
	parser_car_t.add_argument('-tcn', '--tissue_number', dest='tissue_number', type=int, help='maximum tolerable number of tissues that are allowed to be higher than the given CPM expression threshold (default = 3)', default = 3)
	...
	parser_tcr.add_argument('-ic', '--isoform_cpm_inf', dest='isoform_cpm_inf', type=str, help='Isoform CPM infile', required=True)
	parser_tcr.add_argument('-ip', '--isoform_proportion_inf', dest='isoform_proportion_inf', type=str, help='Isoform proportion infile', required=True)
	...
	parser_tcr.add_argument('-tcn', '--tissue_number', dest='tissue_number', type=int, help='maximum tolerable number of tissues that are allowed to be higher than the given CPM expression threshold (default = 3)', default = 3)
	...
	parser_tcr.add_argument('-aic', '--annotated_isoform_contri_inf', dest='annotated_isoform_contri_inf', type=str, help='file generated before, which ends with \"_annotated_isoform_contribution.txt\"', required=True)
	parser_tcr.add_argument('-tci', '--trans_CDS_inf', dest='trans_CDS_inf', type=str, help='file generated before, format of which is like  \"4_4_*_detailed_match_ID.txt\"', required=True)
	...
	parser_specificity.add_argument('-pi', '--protein_inf', dest='protein_inf', type=str, help='Generated protein fasta file, such as 4_4_XXX_PC.fasta', required=True)
	parser_specificity.add_argument('-ic', '--isoform_cpm_inf', dest='isoform_cpm_inf', type=str, help='Isoform CPM infile', required=True)
	parser_specificity.add_argument('-cs', '--cell_surface_inf', dest='cell_surface_inf', type=str, help='File like 5_3_XXX_high_confidence.txt', required=True)
	...
	parser_topology.add_argument('-pi', '--protein_inf', dest='protein_inf', type=str, help='Generated protein fasta file, such as 4_4_XXX_PC.fasta', required=True)
	parser_topology.add_argument('-sc', '--score_cutoff', dest='score_cutoff', type=float, help='Specificity score cutoff (default = 4)', default = 4)
...
def Sub_process(dir_path, args):
	...
	cmd_process_1 = f"python {dir_path}/scripts/1_1_gtf2bed_1_to_1_based.py {espresso_gtf} {espresso_abundance} {outf_dir}"
	print (cmd_process_1)
	os.system(cmd_process_1)
	# 1.2 calculate CPM based on sam/ban files
	cmd_process_2 = f"python {dir_path}/scripts/1_2_calculate_CPM_based_on_bam.py {espresso_abundance} {folder_sam} {normalized_mode}"
	print (cmd_process_2)
...
def Sub_difftest(dir_path, args):
	...
	detest_p = args.detest_p
	detest_tumor_cpm = args.detest_tumor_cpm
	detest_fc = args.detest_fc
	pretest_p = args.pretest_p
	pretest_tumor_cpm = args.pretest_tumor_cpm
	pretest_tissue_cpm = args.pretest_tissue_cpm
	outf_dir = args.outf_dir.rstrip('/')
	## DE test ##
	cmd_difftest_1 = f"python {dir_path}/scripts/3_1_Tumor_vs_normal_DE_test.py --CPM_inf {isoform_cpm_inf} --Tumor_num {tumor_num} --cutoff_p {detest_p} --cutoff_tumor_cpm {detest_tumor_cpm} --cutoff_fc {detest_fc} --outf_dir {outf_dir}"
	print(cmd_difftest_1)
	os.system(cmd_difftest_1)
	## prevalence test ##
	cmd_difftest_2 = f"python {dir_path}/scripts/3_2_Tumor_vs_normal_prevalence.py --CPM_inf {isoform_cpm_inf} --Tumor_num {tumor_num} --cutoff_p {pretest_p} --cutoff_tumor_cpm {pretest_tumor_cpm} --cutoff_tissue_cpm {pretest_tissue_cpm} --outf_dir {outf_dir}"
...
def Sub_CAR_T(dir_path, args):
	...
	tissue_number = args.tissue_number
	isoform_proportion_inf = args.isoform_proportion_inf
	...
	command_2 = f"python {dir_path}/scripts/5_2_uniprot_annotation_infer.py {isoform_cpm_inf} {protein_inf} {genome} {outf_dir} {out_file}"
	print (command_2)
	os.system(command_2)
	...
	command_4_a = f"python {dir_path}/scripts/5_4_Tumor_vs_normal_CAR_T_summary.py --outf_dir {outf_dir} --outf_key high_confidence --protein_inf {protein_inf} --cell_surface_inf {outf_dir}/5_3_{out_file}_high_confidence.txt"
	print (command_4_a)
	os.system(command_4_a)
	command_4_b = f"python {dir_path}/scripts/5_4_Tumor_vs_normal_CAR_T_summary.py --outf_dir {outf_dir} --outf_key less_confidence --protein_inf {protein_inf} --cell_surface_inf {outf_dir}/5_3_{out_file}_less_confidence.txt"
	print (command_4_b)
	...
	command_5_pre = f"python {dir_path}/scripts/5_5_prioritize_pre.py {tumor_num} {tissue_cpm} {tissue_number} {isoform_cpm_inf} {isoform_proportion_inf} {annotated_isoform_contri_inf} {outf_dir}/5_4_Tumor_vs_normal_CAR_T_high_confidence.txt {genome} {outf_dir}"
	print (command_5_pre)
	os.system(command_5_pre)

	command_6_post = f"python {dir_path}/scripts/5_6_prioritize_post.py {tumor_num} {specificity_score} {isoform_cpm_inf} {protein_inf} {trans_CDS_inf} {outf_dir}"
...
def Sub_TCR(dir_path, args):
	...
	command_6_1 = f"python {dir_path}/scripts/6_1_perform_netMHCpan.py {netMHCpan_dir} {protein_inf} {HLA_str} {window_size} {outf_dir}"
	print (command_6_1)
	...
	command_6_3 = f"python {dir_path}/scripts/6_3_final_prioritize.py {tumor_num} {specificity_score} {tissue_cpm} {tissue_number} {binding_affi} {isoform_proportion_inf} {annotated_isoform_contri_inf} {window_size} {genome} {outf_dir}"
	print (command_6_3)
