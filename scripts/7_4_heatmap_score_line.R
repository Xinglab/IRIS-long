...
inf_name = paste(out_dir, '/7_3_calculate_score_matrix_WindowSize_', window_size,'_', target_trans, '_with_topology.txt',sep='')

fig_data <- read.table(inf_name, header = T,sep='\t', row.names=1, check.names = FALSE)
outf_name = paste(out_dir, '/7_4_heatmap_', target_trans,'_score_line_log2CPM.png', sep='')
outf_pdf_name = paste(out_dir, '/7_4_heatmap_', target_trans, '_score_line_log2CPM.pdf', sep = '')
#dim(fig_data)
...
col_fun = colorRamp2(c(0, max_value/2, max_value), c("#235789", "white", "#C1292E"))
#col_fun = colorRamp2(c(0, max_value), c("white", "#C1292E"))   #single red
...
ha = HeatmapAnnotation(
    Specificity = anno_lines(median_row, gp=gpar(col = '#8E44AD', lwd=2), add_points=FALSE, pt_gp=gpar(col = '#8E44AD'), pch=c(1), height = unit(0.8, "in"), ylim = c(max(min_value_ha,0), max_value_ha), extend = 0.05),
...
f_fig = Heatmap(df,
	heatmap_legend_param = list(
		title = "Expression of transcripts, log2(CPM+1)", title_gp = gpar(fontsize = font_size), labels_gp = gpar(fontsize = font_size), color_bar = "continuous", legend_direction="horizontal", title_position = "topleft", legend_width = unit(1.5, "in")
		), 
...
png(file=outf_name, width=6, height=7, units='in',res=1200)
#draw(f_fig, heatmap_legend_side = "top", annotation_legend_side = "top", merge_legend =TRUE)
fig_2
dev.off()

pdf(file=outf_pdf_name, width=6, height=7)
#draw(f_fig, heatmap_legend_side = "top", annotation_legend_side = "top", merge_legend =TRUE)
fig_2
dev.off()
