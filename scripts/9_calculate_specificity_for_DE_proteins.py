...
required_trans_list = []
with open("%s/3_1_Tumor_vs_normal_DE_test.txt" % outf_dir, "r") as cand_trans_inf:
...
outf = open("%s/9_Tumor_vs_normal_DE_test_with_tumor_specificity.txt" % outf_dir, 'w')
with open("%s/3_1_Tumor_vs_normal_DE_test.txt" % outf_dir, "r") as cand_trans_inf_2:
	for index,line in enumerate(cand_trans_inf_2):
		line = line.strip()
		arr = line.split('\t')
		if index == 0: 
			outf.write(arr[0]+'\t'+arr[1]+'\t'+'\t'.join(arr[2:len(arr)])+'\tPeptide_with_high_tumor_specificity\tPos_pep_with_high_tumor_specificity\tTumor_specificity_score_for_each_regions\tPeptide_genomic_position\n')
			continue
		else:
			pep_list = []
			pos_list = []
			score_list = []
			if arr[0] not in trans2seq_dict: continue
			...
			if len(score_list) > 0:
				...
				genome_loc_list = []
				for peptide_loc in pos_list:
					genome_loc = get_geome_loc(chrom, strand, peptide_loc, cds)
					genome_loc_list.append(genome_loc)

				if len(pos_list) > 0:
					outf.write(arr[0]+'\t'+arr[1]+'\t'+'\t'.join(arr[2:len(arr)])+'\t'+';'.join(concat_pep_list)+'\t'+';'.join(concat_pos_list)+'\t'+';'.join(concat_score_list)+'\t'+';'.join(genome_loc_list)+'\n')
outf.close()
